	        ...
	        @foreach($data as $item)
	        	...
	        	<tr>
	        		...
	        		<td style="border: 1px solid black; border-collapse: collapse; text-align: center;">
	        			<a href="{{ route('changeCat', ['topic' => $item['id'] ]) }}">{{ $item['topic'] }}</a>
	        		</td>
	        		<td style="border: 1px solid black; border-collapse: collapse; text-align: center;">
	        			<a href="{{ route('changeQuestion', ['topic' => $item['id'] ]) }}">{{ $item['question'] }}</a>
	        		</td>
	        		<td style="border: 1px solid black; border-collapse: collapse; text-align: center;">
	        			<a href="{{ route('changeAnswer', ['topic' => $item['id'] ]) }}">{{ $item['answer'] }}</a>
	        		</td>
	        		...
	        		<td style="border: 1px solid black; border-collapse: collapse; text-align: center;">
	        			<form method="POST" action="{{ route('topicsCategory', ['topic' => $item['id'] ]) }}">
							@csrf
	        		...
	        		<td style="border: 1px solid black; border-collapse: collapse; text-align: center;">
						...
						@elseif( $item['change_status'] == 'Answer' )
							...
							<a style="color: green;" href="{{ route('answer', ['topic' => $item['id'] ]) }}">{{ $item['change_status'] }}</a>
